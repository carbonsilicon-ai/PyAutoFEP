...
def CheckMol(path,path_valid_mols,path_valid_info,path_invlid_info):
    ...
    if not os.path.exists(path):
        ...
        exit()
    if not os.path.exists(path_valid_mols):
        os.makedirs(path_valid_mols)
        #print('No Valid Ligands Pathway was found ')
        #exit()
    ...
    for mol_name in mols:   # check chemistry
        try:
            ...
            if style == 'mol':
                ...
                mol = Chem.MolFromMolFile(mol_path,sanitize=True,removeHs=False)
            elif  style == 'sdf':
                mol = Chem.SDMolSupplier(mol_path,sanitize=True,removeHs=False)[0]
            else:
    ...
    charge_valid_groups, charge_invalid_groups,valid_charge = compare_charge(valid_Chemical_Rule)
...
def write_json(file_path):
    path_info = {
    "path": 'lig_data',
    "path_valid_mols": 'lig_data_output',
    "path_valid_info": 'valid_info.csv',
    "path_invalid_info": 'invalid_info.csv',
    }
...
if __name__ == '__main__':
    ...
    file_path = 'check_ligs_input.json'
